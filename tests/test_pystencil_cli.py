...
# First party modules
from pystencil import pystencil_cli


def test_author() -> None:
    """Test function to assert correct author name."""
    runner = CliRunner()
    result = runner.invoke(pystencil_cli.author)
...
def test_author_verbose() -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(pystencil_cli.author, ["--verbose"])
    assert result.exit_code == 0
    assert (
...
def test_author_help() -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(pystencil_cli.author, ["--help"])
    assert result.exit_code == 0
